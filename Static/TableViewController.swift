...
public class TableViewController: UIViewController {
    ...
    private var initialLoadOnceToken = Int()
    private func performInitialLoad() {
        tableView.reloadData()
    }

    private func clearSelectionsIfNecessary(_ animated: Bool) {
    ...
    private func selectRowsAtIndexPaths(_ indexPaths: [IndexPath], animated: Bool) {
        indexPaths.forEach { tableView.selectRow(at: $0, animated: animated, scrollPosition: .none) }
    }

    private func deselectRowsAtIndexPaths(_ indexPaths: [IndexPath], animated: Bool) {
